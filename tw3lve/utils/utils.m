...
void saveOffs() {
    ...
    #define CACHEOFFSET(offset, name) CACHEADDR(GETOFFSET(offset), name)
        ...
        CACHEOFFSET(extension_add, "JBD_ExtAdd");
        CACHEOFFSET(extension_create_file, "JBD_ExtCreate");
        CACHEOFFSET(extension_release, "JBD_ExtRel");
        CACHEOFFSET(strlen, "JBD_Strlen");
    
        dictionary[@"JBD_KernProc"] = ADDRSTRING(ReadKernel64(ReadKernel64(GETOFFSET(kernel_task)) + koffset(KSTRUCT_OFFSET_TASK_BSD_INFO)));
        LOGME("JBD_KernProc = %@",  ADDRSTRING(ReadKernel64(ReadKernel64(GETOFFSET(kernel_task)) + koffset(KSTRUCT_OFFSET_TASK_BSD_INFO))));
...
void getOffsets() {
    ...
    GO(extension_add);
    GO(extension_create_file);
    GO(extension_release);
    GO(strlen);
...
void extractLdid()
{
    NSString *substrateFile = get_path_res(@"bootstrap/sileo/ldid.tar");
    ArchiveFile *subBSFile = [ArchiveFile archiveWithFile:substrateFile];
    [subBSFile extractToPath:@"/"];
...
{
    NSString *substrateFile = get_path_res(@"bootstrap/sileo/jbres.tar");
    ArchiveFile *subBSFile = [ArchiveFile archiveWithFile:substrateFile];
    [subBSFile extractToPath:@"/"];
...
void finishSileo()
{
    ...
    trust_file(@"/jb/jelbrekd");
    trust_file(@"/bin/launchctl");
    trust_file(@"/bin/_launchctl");
    trust_file(@"/usr/lib/amfid_payload.dylib");
    trust_file(@"/usr/lib/pspawn_hook.dylib");
    trust_file(@"/usr/lib/libjailbreak.dylib");
    trust_file(@"/usr/lib/libsubstitute.dylib");
    trust_file(@"/usr/libexec/ldid");
    trust_file(@"/usr/libexec/ldid_wrapper");
    
    execCmd("/usr/bin/uicache", NULL);
    
    _assert(execCmd("/bin/launchctl", "load", "/Library/LaunchDaemons/jelbrekd.plist", NULL) == ERR_SUCCESS, @"Failed To Load Jelbrekd!", true);
    
    if (!pspawnHookLoaded())
    {
        _assert(create_patched_executable_with_dylib("/usr/libexec/xpcproxy", "/usr/lib/pspawn_hook.dylib") == ERR_SUCCESS, @"Failed To Load Jelbrekd!", true);
